...
require_contains ".config/hypr/startup.conf" 'wl-clip-persist --clipboard regular' "startup persists regular Wayland clipboard"
...
require_contains ".config/hypr/keybindings.conf" '^[[:space:]]*bind = \$mainMod, period, exec, .*tnctl rofi emoji$' "super period opens emoji picker"
...
require_contains ".config/hypr/scripts/keybinds.sh" 'SUPER \+ \.[[:space:]]+Emoji picker' "keybind help shows emoji picker"
...
require_contains "verify-rice.sh" '\[ -e "\$command_name" \]' "verify-rice accepts absolute package artifact paths"
